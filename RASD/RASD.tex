...
\subsection{Scope:}
In this section the so called "world and machine phenomena" is introduced.\\
We distinguish the world and the machine. The world is related to every phenomenon that takes place outside of the region of events that the S2B (called machine) is able to control or eventually observe. Instead, the machine is related to phenomena which happen inside the machine and so they are completely controllable. Some of the totality of the phenomena can be observed by the machine but they're controlled by the world and vice versa. These are called shared phenomena.
...
It's the situation in which the user sees a violation which can concern traffic or parking. The user knows what happens and also where and when. Such violations could be for example a car/motorcycle parked in a red zone or on a sidewalk area and so on.
...
\begin{figure}[h
    ...
    \includegraphics[scale=0.4]{State_chart.png}
	\centering
...
\begin{itemize}
	...
	\item $[\textbf{D8}]$: The municipality provides the reports (sent by the users) to the authorities automatically.
	\item $[\textbf{D9}]$: The email identifies uniquely a private user.
	\item $[\textbf{D10}]$: Through the fiscal code it is possible to identify uniquely a private user.
	\item $[\textbf{D11}]$: The username of a private user is a unique identifier of him/her.
	\item $[\textbf{D12}]$: It is not possible to have streets that are in the same city and have the same name.
	\item $[\textbf{D13}]$: A street can be identified through its name and the city in which it is located.
	\item $[\textbf{D14}]$: A vehicle can be identified through its plate.
...
\begin{itemize}
	...
	\item IOS
\end{itemize}	
The choice of that decision is the fact that these operating systems are the most common ones.
...
\item The system does not have enough data to answer to the request;
\end{itemize}
...
\begin{itemize}
 ...
 \begin{itemize}
 	...
 	\item $[D11]$: The username of a private user is a unique identifier of him/her.
 	\item $[D12]$: It is not possible to have streets that are in the same city and have the same name.
	\item $[D13]$: A street can be identified through its name and the city in which it is located.
 ...
 \begin{itemize}
 	...
 	\item $[R30]:$ A street is considered dangerous if and only if  the number of reports is 10\% greater than the average number of violations per street reported by the users or ,as alternative, 10\% greater than the average violation per street retrieved from the municipality DB.
 	\item $[D4]:$ The GPS signal has a relative error of 10 meters.
 	\item $[D7]:$ The internet connection has to be enabled when the app needs it.
 \end{itemize}
...
\item $[D10]$: Through the fiscal code it is possible to identify uniquely a private user.
\item $[D11]$: The username of a private user is a unique identifier of him/her.
 \end{itemize}
...
\end{itemize}
\begin{figure}[h]
	\includegraphics[scale=1.2,angle=90]{Alloy images/staticViewViolations.png}
	\centering
...
The system crosses also the information about accidents of the municipality in order to have better information about the safety.
\paragraph{World 3:}
...
\begin{figure}[h]
	\includegraphics[scale=1.168,angle=90]{Alloy images/staticCompleteView.png}
	\centering
	\caption{Static complete view of the application}
\end{figure}
\FloatBarrier

All the predicates have been validated through the use of the alloy validation tool as you can see in the image below.\\
\begin{figure}[h]
