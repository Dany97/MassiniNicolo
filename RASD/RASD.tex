...
\item $[goal 5]$: The system must allow the user to check the safety of a street, a group of streets or a parking area that he/she wants.
...
\begin{itemize}
	...
	\item \textbf{Personal data =} Data belonging to the user, needed in the moment of the signing-up. They are name, surname, username and e-mail.
...
\textbf{2)} Streets with no parking lots for disabled can become impracticable for disabled car drivers. Maybe he/she would not have enough space for a wheelchair in a non-disabled car park. Instead for non-disabled ones this aspect does not generate any problem for sure. \\ \\
For example they will have information about cars parked in a handicap parking or on ramps for disabled people.\\
...
\begin{itemize}
	...
	\item $[\textbf{D12}]$: A street can be identified with its name and the city of which it belongs.
...
\begin{itemize}
 ...
 \begin{itemize}
 	...
 	\item $[R30]:$ The chosen username must identify the private user uniquely.
...
In this part we will indicate the performance requirements of SafeStreets. Our application will use cloud databases running in a cloud computing platform purchased from third parties. This choice was made to try to increase the elasticity on the number of requests. In fact, through this approach it is possible to balance the requests during the service. It is possible to decrease resources when the load is low and to increase them when it is high.\\
From the dynamic point of view we need that the information about streets, violations and vehicles is send and updated quite fast, so the violations are sent to the municipality in maximum 10 minutes from when they have been reported by a user, while the maps that show the safest area and parking areas have to be updated in maximum 1 hour. Same thing for the reported vehicles.
